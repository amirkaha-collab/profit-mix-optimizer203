...
def render_mix_builder(df_active, all_funds_list):
    ...
    with st.expander("⚙️ הגדרות מתקדמות", expanded=False):
        ...
        # Managers unchecked by default on first load (user may re-select manually)
        _DEFAULT_UNCHECKED_MANAGERS = {"מנורה", "IDI", "אי.די.אי", "אינפיניטי"}
        _mgr_state = st.session_state.get("selected_managers")
        if _mgr_state is None:
            # First load — exclude default-unchecked managers
            real_sel_mgrs = [m for m in all_mgrs_full if m not in _DEFAULT_UNCHECKED_MANAGERS]
        else:
...
<script>
  setTimeout(function(){
    ...
    with st.expander("⚖️ השוואת מסלולי השקעה", expanded=auto_open):
        ...
        num_cols = ['חו"ל (%)','מניות (%)', 'מט"ח (%)','לא־סחיר (%)','שארפ','שירות', 'תשואה 12 חודשים']
                ...
                if col in num_cols:
                    try:
                        ...
                        if math.isnan(f):
                            ...
                            cells += f"<td style='text-align:center'>{f:.{dec}f}{unit}</td>"
                    except:
                        cells += "<td style='text-align:center'>—</td>"
                else:
